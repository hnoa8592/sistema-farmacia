...
export class FormasComponent implements OnInit {
  ...
  cargar(): void {
    this.cargando = true;
    this.catalogoService.getFormas().subscribe({ next: d => { this.items = d; this.cargando = false; }, error: () => { this.cargando = false; } });
  }
  ...
  guardar(): void {
    ...
    this.guardando = true;
    const op = this.itemSeleccionado ? this.catalogoService.editarForma(this.itemSeleccionado.id, this.form.value) : this.catalogoService.crearForma(this.form.value);
  ...
  eliminar(item: CatalogoDTO): void {
    this.confirmationService.confirm({ message: `¿Eliminar "${item.nombre}"?`, accept: () => {
      this.catalogoService.eliminarForma(item.id).subscribe({ next: () => { this.messageService.add({ severity: 'success', summary: 'Eliminado' }); this.cargar(); }, error: () => {} });
    }});
  }
}
